...
<template>
    <div class="max-w-7xl mx-auto p-6">
        <div class="flex justify-between items-center mb-6">
            ...
            <Button label="Створити список" icon="pi pi-plus" @click="openModal" severity="success" />
        </div>

        <!-- Table -->
        <DataTable :value="userStore.propertyLists" :loading="loading" stripedRows showGridlines :paginator="true" :rows="10">
            ...
            <Column header="Об'єкти">
                <template #body="slotProps">
                    <div v-if="slotProps.data.properties?.length > 0">{{ slotProps.data.properties.length }} об'єкта(-ів)</div>
                    <div v-else>Немає об'єктів</div>
                </template>
            </Column>
            <Column header="Дії">
                <template #body="slotProps">
                    <Button icon="pi pi-eye" class="p-button-rounded p-button-info mr-2" @click="viewList(slotProps.data)" tooltip="Переглянути список" />
                    <Button icon="pi pi-trash" class="p-button-rounded p-button-danger" @click="confirmDelete(slotProps.data)" tooltip="Видалити список" />
                </template>
            </Column>
        </DataTable>

        <!-- Modal -->
        <Dialog v-model:visible="displayModal" modal header="Створення нового списку" :style="{ width: '50vw' }" :breakpoints="{ '960px': '75vw', '641px': '90vw' }">
            <form @submit.prevent="createNewList" class="space-y-4">
                <div class="field">
                    <label for="name" class="block mb-1">Назва списка</label>
                    <InputText id="name" v-model="newList.name" type="text" class="w-full" required />
                </div>

                <div class="field">
                    <label for="client" class="block mb-1">Клієнт</label>
                    <Select id="client" v-model="newList.client" :options="clients" optionLabel="name" placeholder="Вибрати клієнта" class="w-full" required />
                </div>

                <div class="font-semibold text-xl">Тип нерухомості</div>
                <Select id="categoryProperty" name="categoryProperty" v-model="newList.category" :options="dropdowns.category" optionLabel="name" placeholder="Select" required />

                <div class="font-semibold text-xl">Мета використання</div>
                <Select name="subcategoryProperty" v-model="newList.subcategory" :options="dropdowns.subcategory" optionLabel="name" placeholder="Select" required />
            ...
            <template #footer>
                <Button label="Скасувати" icon="pi pi-times" @click="closeModal" class="p-button-text" />
                <Button label="Зберегти" icon="pi pi-check" @click="createNewList" autofocus />
            </template>
        </Dialog>

        <!-- Delete Confirmation Dialog -->
        <Dialog v-model:visible="deleteDialogVisible" modal header="Підтвердження видалення" :style="{ width: '450px' }">
            ...
            <template #footer>
                <Button label="Ні" icon="pi pi-times" @click="deleteDialogVisible = false" class="p-button-text mr-4" />
                <Button label="Так" icon="pi pi-check" @click="deleteList" class="p-button-danger" autofocus />
            </template>
        </Dialog>
    </div>
</template>
