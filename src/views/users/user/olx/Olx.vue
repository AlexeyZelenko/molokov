...
<template>
    <div class="p-4 max-w-md mx-auto space-y-4">
        ...
        <div>
            <label for="region-select" class="block text-sm font-medium text-gray-700 mb-2"> Виберіть регіон </label>
            <select
                id="region-select"
                v-model="selectedRegionId"
                @change="fetchCities"
                :disabled="isLoading"
                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        <div v-if="selectedRegionId" class="space-y-2">
            <label class="block text-sm font-medium text-gray-700 mb-2"> Міста регіону </label>
            ...
            <div v-else-if="regionCities && regionCities.length > 0" class="space-y-4">
                ...
                <div v-for="(districtCities, district) in citiesByDistrict" :key="district" class="border border-gray-200 rounded-md overflow-hidden">
                    ...
                    <div class="divide-y divide-gray-200">
                        <div v-for="city in districtCities" :key="city.id" class="border-t border-gray-200 first:border-t-0">
                            <button
                                @click="toggleCityDetails(city.id)"
                                :disabled="cityDetailsLoading"
                                class="w-full px-4 py-2 text-left bg-white hover:bg-gray-50 transition-colors duration-300 flex items-center justify-between disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                <span>{{ city.name }}</span>
                                <div class="flex items-center">
                                    <svg v-if="cityDetailsLoading && currentLoadingCityId === city.id" class="animate-spin h-4 w-4 text-gray-400 ml-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                    <svg class="h-5 w-5 text-gray-400 transform transition-transform duration-300 ml-2" :class="{ 'rotate-180': selectedCityId === city.id }" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd" />
                                    </svg>
                                </div>
                            </button>

                            <div v-if="selectedCityId === city.id && cityDetails" class="p-4 bg-gray-50 rounded-b-md animate-fade-in">
            ...
            <div v-else-if="!isCitiesLoading" class="text-gray-500 text-center py-4">Немає доступних міст для цього регіону</div>
        </div>

        <div v-if="error" class="text-red-500 mt-4 bg-red-50 p-3 rounded animate-shake"><strong>Помилка:</strong> {{ error }}</div>
    </div>
</template>
