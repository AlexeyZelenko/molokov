...
const subcategoryName = computed(() => storeAreas.realEstateItems.find(item => item.key === 'houses')?.actions.find(subcategory => subcategory.type === 'sell')?.label);
                        ...
                        <div v-for="(item, index) in slotProps.items" :key="index">
                            <div class="flex flex-col sm:flex-row sm:items-center p-6 gap-4"
                                ...
                                <div class="md:w-40 relative">
                                    <img class="block xl:block mx-auto rounded w-full" :src="item.images[0]"
                                         :alt="item.title"/>
                                </div>
                                <div class="flex flex-col md:flex-row justify-between md:items-center flex-1 gap-6">
                                    <div class="flex flex-row md:flex-col justify-between items-start gap-1">
                                        <div class="flex flex-col justify-between gap-1">
                                            ...
                                            <div class="font-small text-surface-500 dark:text-surface-400 text-sm">
                                                {{ item.address.city.name || item.address.city }}  {{ item.address.area.name }}
                                            </div>
                                            <div class="font-small text-surface-500 dark:text-surface-400 text-sm">
                                                {{ formatFirebaseTimestampToTime(item.createdAt) }}
                                            </div>
                                            <div class="font-small text-surface-500 dark:text-surface-400 text-sm">id:
                                                {{ item.idProperty }}
                                            </div>
                                        </div>
                                    </div>
                                                ...
                                                <AccordionHeader>
                                                        <span class="flex items-center gap-2 w-full">
                                                            <Avatar icon="pi pi-user"
                                                                    ...
                                                                    shape="circle"/>
                                                            <span
                                                                class="font-bold whitespace-nowrap px-2">Користувач</span>
                                                        </span>
                                                </AccordionHeader>
                                                <AccordionContent>
                                                    <div
                                                    ...
                                                    <div v-for="(phone, i) in item.creator?.phone" :key="i"
                                                         class="font-medium text-surface-500 dark:text-surface-400 text-sm pt-2">
                            ...
                            <div
                                ...
                                <div class="bg-surface-50 flex justify-center rounded p-4">
                                    <div class="relative mx-auto">
                                        <img class="rounded w-full" :src="item.images[0]" :alt="item.name"
