...
const categoryName = computed(() => storeAreas.realEstateItems.find(item => item.key === 'land')?.title);
const subcategoryName = computed(() => storeAreas.realEstateItems.find(item => item.key === 'land')?.actions.find(subcategory => subcategory.type === 'exchange')?.label);

const category = computed(() => route.query.category || 'land');
