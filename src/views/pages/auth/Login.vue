...
const handleLogin = async () => {
    ...
    try {
        const user = await authStore.login(sumbit, toast);
            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    ...
                    <div>
                        ...
                        <InputText id="email1" type="email" placeholder="Email address" class="mb-4" fluid v-model="email" @keyup.enter="handleLogin" />

                        <label for="password1" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Password</label>
                        <Password id="password1" v-model="password" placeholder="Password" :toggleMask="true" class="mb-4" fluid :feedback="false" @keyup.enter="handleLogin" />
                        ...
                        <Button :label="loading ? 'вхід...' : 'Увійти'" class="w-full" fluid :disabled="loading" @click="!loading && handleLogin()" />

                        <router-link to="/auth/register" class="flex justify-center mt-4">
                            Немає облікового запису?
                            <div class="text-blue-500 hover:text-blue-600 dark:hover:text-blue-400 ml-2">Зареєструватися</div>
                        </router-link>
                    </div>
                </div>
            </div>
