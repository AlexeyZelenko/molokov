...
<template>
    <div class="card flex flex-col gap-4">
        ...
        <template v-for="(label, key) in roomLabels" :key="key">
            ...
            <InputNumber v-model="modelValue.rooms[key]" showButtons mode="decimal" :class="{ 'p-invalid': showErrors && errors[key] }" :min="0" />
        ...
        <Select v-model="modelValue.planning" :options="dropdowns.planning" optionLabel="name" placeholder="Оберіть планування" :class="{ 'p-invalid': showErrors && errors.planning }" />
        ...
        <Select v-model="modelValue.bathroom" :options="dropdowns.bathroom" optionLabel="name" placeholder="Оберіть планування" :class="{ 'p-invalid': showErrors && errors.bathroom }" />
